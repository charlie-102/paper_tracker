...
A stateful tracker that searches GitHub for reproducible low-level vision repositories with pretrained weights. Tracks repo lifecycle and detects fresh weight releases.

## Features

- **Stateful Tracking**: Maintains `history.json` to track repos across runs
- **Lifecycle Detection**: Tracks state transitions (New -> Coming Soon -> Weights Released)
- **Fresh Release Detection**: Highlights repos where weights were just released
- **Two-Pass Search**: Catches both popular (by stars) and bleeding-edge (by update) repos
- **Delta Logic**: Skips re-scanning stable repos, focuses on watchlist
- **Coming Soon Detection**: Detects "weights will be released" promises
- **Weight Detection**: HuggingFace, GitHub Releases, Cloud Drives, Model extensions
- **Conference Detection**: CVPR, ECCV, ICCV, ICLR, NeurIPS, ICML, AAAI, SIGGRAPH + arXiv
...
- **Automatic**: Every Monday at 8:00 AM UTC
- **Manual**: Actions > Weekly Paper Tracker > Run workflow
...
├── .github/
│   └── workflows/
│       └── weekly_tracker.yml   # GitHub Actions workflow
